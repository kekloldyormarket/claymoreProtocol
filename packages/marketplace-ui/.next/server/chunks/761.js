...
exports.FIXED_CURVE_FEES = 2;
exports.LBC_CURVE_FEES = 3;
const truthy = (value) => !!value;
class MarketplaceSdk {
    constructor(provider, tokenBondingSdk, tokenCollectiveSdk, tokenMetadataSdk) {
    ...
    static init(provider, splTokenBondingProgramId = spl_token_bonding_1.SplTokenBonding.ID, splTokenCollectiveProgramId = spl_token_collective_1.SplTokenCollective.ID) {
        return __awaiter(this, void 0, void 0, function* () {
            return new this(provider, yield spl_token_bonding_1.SplTokenBonding.init(provider, splTokenBondingProgramId), yield spl_token_collective_1.SplTokenCollective.init(provider, splTokenCollectiveProgramId), yield spl_utils_1.SplTokenMetadata.init(provider));
        });
    }
    createManualTokenInstructions({ mintKeypair = web3_js_1.Keypair.generate(), decimals, metadata, amount, payer = this.provider.wallet.publicKey, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            instructions.push(spl_token_1.Token.createMintToInstruction(spl_token_1.TOKEN_PROGRAM_ID, mint, ata, publicKey, [], 1000));
            return {
                instructions,
                signers: [mintKeypair],
                output: { mint }
     ...
     * @returns
     */
    disburseCurveInstructions({ tokenBonding, destination, includeRetrievalCurve, }) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            const instructions = [];
            const signers = [];
     ...
     * @returns
     */
    createMarketItemInstructions({ payer = this.provider.wallet.publicKey, seller = this.provider.wallet.publicKey, metadata, metadataUpdateAuthority = seller, quantity, price, bondingArgs, baseMint, targetMintKeypair = web3_js_1.Keypair.generate(), iAmAFreeloader, protocolFee = exports.FIXED_CURVE_FEES, }) {
        return __awaiter(this, void 0, void 0, function* () {
            if (protocolFee == 0 && !iAmAFreeloader) {
                throw new Error("Must use `iAmAFreeloader` flag when setting protocolFee");
            }
            ...
            const { output: { tokenBonding }, instructions: tokenBondingInstructions, signers: tokenBondingSigners, } = yield this.tokenBondingSdk.createTokenBondingInstructions(Object.assign(Object.assign({ payer, reserveAuthority: seller, generalAuthority: seller, curveAuthority: seller, targetMint, mintCap: quantity
                    ...
                    : undefined, ignoreExternalReserveChanges: true, ignoreExternalSupplyChanges: true, sellFrozen: true, buyBaseRoyaltyPercentage: iAmAFreeloader ? 0 : protocolFee, buyBaseRoyaltiesOwner: exports.FEES_WALLET, sellBaseRoyaltyPercentage: 0, sellTargetRoyaltyPercentage: 0, buyTargetRoyaltyPercentage: 0, baseMint, targetMintDecimals: 0 }, bondingArgs), { curve: curve }));
            return {
                output: {
                    tokenBonding,
                    targetMint,
     ...
     * @returns
     */
    createLiquidityBootstrapperInstructions({ payer = this.provider.wallet.publicKey, authority = this.provider.wallet.publicKey, targetMint, targetMintKeypair, metadata, metadataUpdateAuthority = authority, interval, startPrice, minPrice, maxSupply, bondingArgs, baseMint, iAmAFreeloader, protocolFee = exports.LBC_CURVE_FEES, }) {
        return __awaiter(this, void 0, void 0, function* () {
            if (protocolFee == 0 && !iAmAFreeloader) {
                throw new Error("Must use `iAmAFreeloader` flag when setting protocolFee");
            }
            ...
            const { output: { tokenBonding, targetMint: bondingTargetMint }, instructions: tokenBondingInstructions, signers: tokenBondingSigners, } = yield this.tokenBondingSdk.createTokenBondingInstructions(Object.assign({ payer, curve: curve, reserveAuthority: authority, generalAuthority: authority, ignoreExternalReserveChanges: true, ignoreExternalSupplyChanges: true, targetMint, sellBaseRoyaltyPercentage: 0, sellTargetRoyaltyPercentage: 0, buyTargetRoyaltyPercentage: 0, baseMint, advanced: {
                    initialSupplyPad,
                    initialReservesPad,
                }, mintCap: (0, spl_token_bonding_1.toBN)(maxSupply, decimals), buyBaseRoyaltyPercentage: iAmAFreeloader ? 0 : protocolFee, buyBaseRoyaltiesOwner: exports.FEES_WALLET }, bondingArgs));
            return {
                output: {
                    tokenBonding,
     ...
     */
    createRetrievalCurveForSetSupplyInstructions({ supplyAmount, supplyMint, source = this.provider.wallet.publicKey, fixedCurve = new web3_js_1.PublicKey(MarketplaceSdk.FIXED_CURVE), reserveAuthority = this.provider.wallet.publicKey, targetMint, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const instructions = [];
            const signers = [];
            const retrievalInstrs = yield this.tokenBondingSdk.createTokenBondingInstructions({
            ...
            instructions.push(...retrievalInstrs.instructions, spl_token_1.Token.createTransferInstruction(spl_token_1.TOKEN_PROGRAM_ID, source, retrievalInstrs.output.baseStorage, sourceAcctAta.owner, [], new spl_token_1.u64((supplyAmount * Math.pow(10, supplyMintAcc.decimals)).toLocaleString("fullwide", {
                useGrouping: false,
            }))));
            signers.push(...retrievalInstrs.signers);
            return {
     ...
     * @returns
     */
    createRetrievalCurveForSetSupply(args, finality) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.tokenBondingSdk.execute(this.createRetrievalCurveForSetSupplyInstructions(args), args.payer, finality);
        });
    }
    /**
     * Sell `supplyAmount` supply of tokens of `supplyMint` by creating a system of two bonding curves:
    ...
    createTokenBondingForSetSupplyInstructions(_a) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const retrievalInstrs = yield this.createRetrievalCurveForSetSupplyInstructions({
...
function roundPercent(protocolFee) {
    return ((0, spl_utils_1.percent)(protocolFee) || 0) / 4294967295;
}
